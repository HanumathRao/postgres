...
usage() {
  cat <<'EOF'
Usage: scripts/check-leftdeep-varied-queries.sh [options]

Runs a varied query suite (inner joins, LEFT/FULL OUTER joins, subqueries,
derived tables) and validates:
  1) query result equality between enable_left_deep_join=off vs on
  2) on-plan has no right-side join subtree (left-deep shape)

Options:
  --port N            Server port (default: 55432)
  --data-dir PATH     Data directory (default: /tmp/pg_leftdeep)
  --log-file PATH     Log file (default: /tmp/pg_leftdeep_varied.log)
  --rows N            Rows per table (default: 6000)
  --plans-dir PATH    Output dir for captured JSON plans
                      (default: /tmp/leftdeep_varied_plans)
...
LOG_FILE=/tmp/pg_leftdeep_varied.log
ROWS=6000
PLANS_DIR=/tmp/leftdeep_varied_plans
SKIP_BUILD=0
KEEP_RUNNING=0
...
echo "[5/8] Preparing varied test schema..."
...
echo "[6/8] Running varied query suite..."
total=0
fails=0
...
for qf in "${QDIR}"/q*.sql; do
  ...
  if ! check_left_deep_plan "${on_plan}"; then
    echo "    FAIL: on-plan is not left-deep (${on_plan})"
    ((fails += 1))
    continue
  fi

  echo "    PASS: results match; on-plan left-deep"
done

echo "[7/8] Summary: total=${total} failed=${fails} plans_dir=${PLANS_DIR}"
if (( fails > 0 )); then
  exit 1
fi

echo "[8/8] PASS: varied query suite succeeded."
